...
export default function Dashboard() {
  ...
  if (error) {
    return (
      <div className="bg-red-50 text-red-700 rounded-lg p-4 flex items-center gap-2">
        ...
        Falha ao carregar projetos. Verifique se o servidor devctl está rodando.
      </div>
    )
  }
  ...
  if (!projects || projects.length === 0) {
    return (
      <div className="text-center py-20">
        ...
        <h2 className="text-lg font-medium text-slate-900 mb-2">Nenhum projeto ainda</h2>
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-slate-900 mb-6">Projetos</h1>
  ...
  return (
    <div className="bg-white rounded-lg border border-slate-200 p-5 hover:shadow-md transition-shadow">
      ...
      <div className="text-sm text-slate-500 mb-4">
        {project.services?.length || 0} serviço{(project.services?.length || 0) !== 1 ? 's' : ''}
      </div>
